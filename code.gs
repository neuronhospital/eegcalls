...
/* ===================== v96 PERFORMANCE CACHE ===================== */
function v96CacheKey_(city,type){return 'v96_'+type+'_'+String(city||'ALL');}
function v96CachePut_(city,type,key,row){try{const c=CacheService.getScriptCache();const o=JSON.parse(c.get(v96CacheKey_(city,type))||'{}');o[String(key)]=row;c.put(v96CacheKey_(city,type),JSON.stringify(o),21600);}catch(e){}}
function v96CacheGet_(city,type,key){try{const o=JSON.parse(CacheService.getScriptCache().get(v96CacheKey_(city,type))||'{}');return o[String(key)]||0;}catch(e){return 0;}}
...
function doGet() {
  ...
  return json_({
    ...
    version: "2026-08-24-HTML78-v5-CODE78-v5"
  });
}
...
function doPost(e) {
  try {
    ...
    switch (action) {
      ...
      case "getFollowupCityCache":
        return json_(getFollowupCityCache_());

      case "getRefundPatientByWhatsApp":
        return json_(getRefundPatientByWhatsApp_(body));

      case "saveRefund":
        return json_(saveRefund_(body));
      ...
      case "getEEGBookingPatientsByWhatsApp":
        return json_(getEEGBookingPatientsByWhatsApp_(body));

      case "getEEGBookedPatientsByWhatsApp":
        return json_(getEEGBookedPatientsByWhatsApp_(body));
...
function getOrCreateBookingIndexSheet_(ss,name,headers){
...
function makeBookingDuplicateKey_(city,date,phone){
  return [String(city||'').trim().toLowerCase(),String(date||''),String(phone||'')].join('_');
}
function buildBookingIndexesIfNeeded_(ss){
  const idx=getOrCreateBookingIndexSheet_(ss,'Booking_Index',['Key','Row','City']);
  const req=getOrCreateBookingIndexSheet_(ss,'Booking_Request_Index',['RequestId','Row','City']);
  if(idx.getLastRow()>1 && req.getLastRow()>1) return;
  const cities=ss.getSheets().map(s=>s.getName()).filter(n=>!['Booking_Index','Booking_Request_Index'].includes(n));
  const idxRows=[], reqRows=[];
  cities.forEach(city=>{
    ...
    rows.forEach((r,i)=>{
      const rowNo=i+2;
      if(r[0]) idxRows.push([makeBookingDuplicateKey_(city,normalizeDisplayedBookingDateDDMMYYYY_(r[1]),normalizePhone_(r[7])),rowNo,city]);
      if(r[17]) reqRows.push([String(r[17]),rowNo,city]);
    });
  });
  if(idxRows.length) idx.getRange(2,1,idxRows.length,3).setValues(idxRows);
  if(reqRows.length) req.getRange(2,1,reqRows.length,3).setValues(reqRows);
}
function cacheBookingIndex_(key,value){
  try{
    CacheService.getScriptCache().put(String(key),JSON.stringify(value),21600);
  }catch(e){}
}

function getCachedBookingIndex_(key){
  try{
    const v=CacheService.getScriptCache().get(String(key));
...
function addBookingIndexes_(ss,city,date,phone,requestId,row){
  const idx=getOrCreateBookingIndexSheet_(ss,'Booking_Index',['Key','Row','City']);
  const req=getOrCreateBookingIndexSheet_(ss,'Booking_Request_Index',['RequestId','Row','City']);

  const duplicateKey=makeBookingDuplicateKey_(city,date,phone);
  const rows=[
    [duplicateKey,row,city]
  ];

  idx.getRange(idx.getLastRow()+1,1,rows.length,3).setValues(rows);
  req.getRange(req.getLastRow()+1,1,1,3).setValues([[String(requestId),row,city]]);

  cacheBookingIndex_("dup:"+duplicateKey,{row:row,city:city});
  cacheBookingIndex_("req:"+String(requestId),{row:row,city:city});
}

function findDuplicatePatientBooking_(ss,city,phone,age,ageUnit,name,today){
  buildBookingIndexesIfNeeded_(ss);
  const key=makeBookingDuplicateKey_(city,today,phone);
  let cached=getCachedBookingIndex_("dup:"+key);

  if(!cached){
    const idx=ss.getSheetByName('Booking_Index');
    if(!idx || idx.getLastRow()<2) return null;
    const data=idx.getRange(2,1,idx.getLastRow()-1,3).getValues();
    for(const r of data){
      if(r[0]===key){
        cached={row:Number(r[1]),city:r[2]};
        cacheBookingIndex_("dup:"+key,cached);
        break;
      }
    }
  }
  ...
  const sh=ss.getSheetByName(cached.city);
  const rowNo=Number(cached.row);
  const row=sh.getRange(rowNo,1,1,HEADERS.length).getValues()[0];
  if(Number(parseAgeValue_(row[4]))!==Number(age)) return null;
  if(String(parseAgeUnit_(row[4])).toLowerCase()!==String(ageUnit).toLowerCase()) return null;
  if(!namesLikelySame_(name,row[3])) return null;

  return {row:row,rowNumber:rowNo,sheet:sh,appointmentId:row[0],date:row[1],patientName:row[3],age:parseAgeValue_(row[4]),ageUnit:parseAgeUnit_(row[4]),city:row[8]||city,patientType:row[6],address:row[5]||'',opdCharges:row[11],nextFollowupCity:row[10]||''};
}
...
function bookAppointment_(data) {
  ...
  try {
    ...
    /*
     * New bookings are appended after the existing data rows.
     * This avoids the row-shifting cost of inserting at the top of a
     * large Google Sheet. Updates use appointment/request identifiers
     * and stored row locations, so they do not depend on row order.
     */
    ...
    try {
      ...
      // Booking path avoids formatting operations to keep write latency low.
    } catch (writeError) {
      try { sheet.deleteRow(newRowNumber); } catch (cleanupError) {}
      throw writeError;
    }
...
function findBookingRequest_(ss, requestId, city) {
  buildBookingIndexesIfNeeded_(ss);
  let cached=getCachedBookingIndex_("req:"+String(requestId));

  if(!cached){
    const idx=ss.getSheetByName('Booking_Request_Index');
    if(!idx || idx.getLastRow()<2) return null;
    const data=idx.getRange(2,1,idx.getLastRow()-1,3).getValues();
    for(const r of data){
      if(String(r[0])===String(requestId)){
        cached={row:Number(r[1]),city:r[2]};
        cacheBookingIndex_("req:"+String(requestId),cached);
        break;
      }
    }
  }

  if(!cached || (city && cached.city!==city)) return null;

  const sh=ss.getSheetByName(cached.city);
  const row=sh.getRange(Number(cached.row),1,1,HEADERS.length).getValues()[0];
  return {appointmentId:row[0],date:row[1],patientName:row[3],age:parseAgeValue_(row[4]),ageUnit:parseAgeUnit_(row[4]),city:row[8]||cached.city,patientType:row[6],address:row[5]||'',whatsapp:normalizePhone_(row[7]),opdCharges:row[11],eegCharges:row[14],bookingRequestId:row[17],eegBookingRequestId:row[18],bookingTimestamp:row[2],nextFollowupCity:row[10]||'',opdCashPaid:row[12],opdOnlinePaid:row[13],opdTotalPaid:(Number(row[12])||0)+(Number(row[13])||0)};
}
...
function updateOPDCharges_(data) {
  requireOPDChargePassword_(data.password);
...
/**
 * EEG BOOKING CANDIDATE RETRIEVAL
 * Returns today's patients for the selected city who have NOT yet booked
 * an EEG. Kept separate from getEEGPatientsByWhatsApp_() so Update OPD and
 * Update EEG workflows retain their existing retrieval behavior.
 */
function getEEGBookingPatientsByWhatsApp_(data) {
  const result = getEEGPatientsByWhatsApp_(data);
  const patients = result.patients.filter(function(patient) {
    // Column S is exposed as eegBookingRequestId. Existing ID = already booked.
    return !String(patient.eegBookingRequestId || '').trim();
  });

  if (!patients.length) {
    throw new Error(
      "No patient awaiting EEG booking was found for this WhatsApp number in " +
      result.city + " for today (" + formatDDMMYYYYForMessage_(getTodayDDMMYYYY_()) + ")."
    );
  }

  return {ok:true, city:result.city, patients:patients};
}

/**
 * EEG UPDATE CANDIDATE RETRIEVAL
 * Returns only today's patients for the selected city who have an existing
 * EEG booking. This is intentionally separate from getEEGPatientsByWhatsApp_()
 * so Update OPD continues to see the normal patient list.
 */
function getEEGBookedPatientsByWhatsApp_(data) {
  const result = getEEGPatientsByWhatsApp_(data);
  const patients = result.patients.filter(function(patient) {
    // Column S is exposed as eegBookingRequestId. An existing request ID
    // confirms that the patient has actually booked an EEG appointment.
    return String(patient.eegBookingRequestId || '').trim() !== '';
  });

  if (!patients.length) {
    throw new Error(
      "No patient with a booked EEG appointment was found for this WhatsApp number in " +
      result.city + " for today (" + formatDDMMYYYYForMessage_(getTodayDDMMYYYY_()) + ")."
    );
  }
 ...
 *
 * The response is built from booking fields and timestamps. Display order
 * is determined by timestamp/date sorting rather than spreadsheet row
 * position, so storage order does not define the user-facing order.
 */
function findPatientsByWhatsApp_(ss, phone, scheduledCities) {
  const scheduled = Array.isArray(scheduledCities) ? scheduledCities : [];
  const scheduledSet = new Set(scheduled.map(normalizeCity_));

  const orderedCities = [];
  scheduled.forEach(function(city) {
    const c = normalizeCity_(city);
    if (c && !orderedCities.includes(c)) orderedCities.push(c);
  });
  ...
  orderedCities.forEach(function(city) {
    ...
    // TextFinder avoids loading the complete city sheet into memory.
    const finder = sheet
    ...
    if (matches.length) {
      groups.push({
        city: city,
        scheduledToday: scheduledSet.has(city),
        matches: matches
      });
  ...
  // Common retrieval must return all matching patients.
  // Workflow-specific filtering (for example excluding today's OPD
  // appointments in follow-up history) must happen in that workflow only.
  // Refund Portal requires today's appointments for refund processing.
  const cleanedMatches = allMatches.sort(function(a,b){return (b.bookingTimestampMs||0)-(a.bookingTimestampMs||0);});

  return {
    matches: cleanedMatches,
    groups: groups.map(function(g){
      g.matches=g.matches.sort(function(a,b){return (b.bookingTimestampMs||0)-(a.bookingTimestampMs||0);}).slice(0,10);
      return g;
...
function findPatientByAppointmentId_(ss, appointmentId, phone, city) {
  const cached=v96CacheGet_(city,'appointment',appointmentId);
  const sh=ss.getSheetByName(city);
  if(cached && sh && cached<=sh.getLastRow()){
    const row=sh.getRange(cached,1,1,HEADERS.length).getValues()[0];
    if(String(row[0]||'')===appointmentId && (!phone || normalizePhone_(row[7])===phone)) return {sheet:sh,rowNumber:cached,row:row};
  }
...
function getTimestampMs_(value) {
  ...
  /*
   * Booking Timestamp can arrive from Google Sheets as a real Date object
   * OR as text such as "24/08/2026 15:07:09". JavaScript's Date parser is
   * not reliable for DD/MM/YYYY text, so parse that format explicitly.
   * This is important for Follow-up ordering: an unparseable timestamp
   * must not silently fall back to spreadsheet row order.
   */
  if (Object.prototype.toString.call(value) === "[object Date]") {
  ...
  if (typeof value === "number" && Number.isFinite(value)) {
    // Defensive support for a Google Sheets serial-date value.
  ...
  const m = s.match(/^(\d{1,2})[\/\-](\d{1,2})[\/\-](\d{4})(?:[ T](\d{1,2}):(\d{2})(?::(\d{2})(?:\.\d+)?)?)?$/);
  if (m) {
  ...
  // ISO/other parseable formats remain supported for compatibility.
 ...
 */
function requireOPDChargePassword_(password) {
  if (String(password == null ? "" : password) !== OPD_CHARGE_PASSWORD) {
...
function retrieveRecords_(data) {
    ...
    function readBlock(startRow, endRow) {
      ...
      block.forEach(function(row) {
        ...
        rows.push({
          ...
          mobileNumber:mobile,
          opdRefund: Number(row[20]) || 0,
          eegRefund: Number(row[21]) || 0,
          date: effectiveDate,
  ...
  let opdTotal=0,opdCash=0,opdOnline=0,opdPaid=0,opdRefund=0;
  let eegTotal=0,eegCash=0,eegOnline=0,eegPaid=0,eegRefund=0,eegCount=0;

  rows.forEach(function(r) {
    opdTotal+=Number(r.opdCharges)||0; opdCash+=Number(r.opdCashPaid)||0; opdOnline+=Number(r.opdOnlinePaid)||0; opdPaid+=Number(r.opdTotalPaid)||0; opdRefund+=Number(r.opdRefund)||0;
    if(r.eegCharges!==null){eegTotal+=Number(r.eegCharges)||0;eegCash+=Number(r.eegCashPaid)||0;eegOnline+=Number(r.eegOnlinePaid)||0;eegPaid+=Number(r.eegTotalPaid)||0;eegRefund+=Number(r.eegRefund)||0;eegCount++;}
  });
  ...
  return {
    ...
    totals: {
      ...
      eegCount: eegCount,
      opdTotal:opdTotal, opdCash:opdCash, opdOnline:opdOnline, opdPaid:opdPaid, opdRefund:opdRefund,
      eegTotal:eegTotal, eegCash:eegCash, eegOnline:eegOnline, eegPaid:eegPaid, eegRefund:eegRefund
    }
  };
}
...
function getPatientHistoryByWhatsApp_(data){
  ...
  const cache=CacheService.getScriptCache();
  const cacheKey="patientHistory_"+phone;
  ...
  const ss=SpreadsheetApp.getActiveSpreadsheet();
  const todayCities=getTodayVisitCities_().cities||[];
  const result=findPatientsByWhatsApp_(ss,phone,todayCities);

  if(!result.matches.length)throw new Error("No patient was found with this WhatsApp number.");

  const groups=result.groups.map(g=>({
    city:g.city,
    scheduledToday:g.scheduledToday,
    patients:g.matches.map(m=>patientObject_(m.row,m.city))
      .filter(p=>normalizeBookingDateDDMMYYYY_(p.date)!==getTodayDDMMYYYY_())
      .sort((a,b)=>(b.bookingTimestampMs||0)-(a.bookingTimestampMs||0))
      .slice(0,10)
  })).filter(g=>g.patients.length);

  const output = {
    ok:true,
    scheduledCities:todayCities,
    groups:groups,
    // Keep the flat array for backward compatibility with any older frontend.
    patients:result.matches.map(m=>patientObject_(m.row,m.city))
      .filter(p=>normalizeBookingDateDDMMYYYY_(p.date)!==getTodayDDMMYYYY_())
      .sort((a,b)=>(b.bookingTimestampMs||0)-(a.bookingTimestampMs||0))
      .slice(0,10)
  };

  cache.put(cacheKey, JSON.stringify(output), 7200);
...
/* ===================== REFUND PORTAL ===================== */
function getRefundPatientByWhatsApp_(data){
  const phone=normalizePhone_(data.whatsapp);
  if(!/^[6-9]\d{9}$/.test(phone)) throw new Error("Invalid WhatsApp number.");
  const ss=SpreadsheetApp.getActiveSpreadsheet();
  const selectedCity=normalizeCity_(data.city);
  const result=findPatientsByWhatsApp_(ss,phone,selectedCity?[selectedCity]:(getTodayVisitCities_().cities||[]));
  const today=getTodayDDMMYYYY_();
  const patients=[];
  result.matches.forEach(function(m){
    const r=m.row;
    if(normalizeBookingDateDDMMYYYY_(r[1])!==today) return;

    const p=patientObject_(r,m.city);
    p.refundRow=m.rowNumber;

    // Spreadsheet refund columns:
    // U (index 20) = OPD Refund
    // V (index 21) = EEG Refund
    const opdRefund = refundAmountNumber_(r[20]);
    const eegRefund = refundAmountNumber_(r[21]);

    // Normalize paid amounts also before comparison.
    const opdPaid = refundAmountNumber_(p.opdTotalPaid);
    const eegPaid = refundAmountNumber_(p.eegTotalPaid);

    p.opdRefund = opdRefund;
    p.eegRefund = eegRefund;

    // A service remains refundable only when paid amount exists and the
    // refund recorded in the spreadsheet has not reached the paid amount.
    // Refund is allowed only once per service.
// Any non-zero refund amount means that service refund is already completed.
    const opdPending = opdPaid > 0 && opdRefund <= 0;
    const eegPending = eegPaid > 0 && eegRefund <= 0;

    // Remove patients only after both OPD and EEG refund eligibility is over.
    if(!opdPending && !eegPending) return;
  ...
  if(!patients.length) throw new Error("No today's refundable appointment found.");
  return {ok:true,patients:patients};
}
function refundAmountNumber_(v){
  if(v===null||v===undefined||v==='') return 0;
  return Number(String(v).replace(/[^0-9.]/g,''))||0;
}

function saveRefund_(data){
  const city=normalizeCity_(data.city),aid=clean_(data.appointmentId);
  const found=findPatientByAppointmentId_(SpreadsheetApp.getActiveSpreadsheet(),aid,normalizePhone_(data.whatsapp),city);
  if(!found) throw new Error("Today's appointment not found.");
  if(normalizeBookingDateDDMMYYYY_(found.row[1])!==getTodayDDMMYYYY_()) throw new Error("Refund allowed only for today's booking.");
  const r=found.row.slice();
  const op=getStoredPayment_(r,12,13,r[11]);
  const eg=r[14]===""?{totalPaid:0}:getStoredPayment_(r,15,16,r[14]);
  const or=Number(data.opdRefund||0), er=Number(data.eegRefund||0);
  if((data.updateOPD && or<=0) || (data.updateEEG && er<=0)) throw new Error("Refund amount must be greater than zero.");
  if(or<0||er<0) throw new Error("Refund cannot be negative.");
  if(or>op.totalPaid) throw new Error("OPD refund cannot exceed OPD paid amount.");
  if(er>eg.totalPaid) throw new Error("EEG refund cannot exceed EEG paid amount.");
  if(data.updateOPD){ r[20]=or; }
  if(data.updateEEG){ r[21]=er; }
  found.sheet.getRange(found.rowNumber,1,1,HEADERS.length).setValues([r]);
  return {ok:true,saved:true,patient:patientObject_(r,city)};
}



function getFollowupCityCache_(){
  const city=normalizeCity_((getTodayVisitCities_().cities||[])[0]||"");
  if(!city)return {ok:false};
  const sh=SpreadsheetApp.getActiveSpreadsheet().getSheetByName(city);
  if(!sh)return {ok:false};
  const last=sh.getLastRow();
  if(last<2)return {ok:true,city:city,rows:[]};
  const start=Math.max(2,last-1499);
  const values=sh.getRange(start,1,last-start+1,HEADERS.length).getValues();
  const rows=values.map(r=>patientObject_(r,city));
  return {ok:true,city:city,rows:rows};
}
